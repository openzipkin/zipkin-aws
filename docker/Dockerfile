...
RUN /code/build-bin/maven/maven_build_or_unjar io.zipkin.aws zipkin-module-aws ${VERSION} module

# zipkin version should match zipkin.version in /code/pom.xml
FROM ghcr.io/openzipkin/zipkin:$zipkin_version as zipkin-aws
LABEL org.opencontainers.image.description="Zipkin with Amazon Web Services on OpenJDK and Alpine Linux"
